...
function groupRoadScores(draws,current,internal){const curG=groupCounts(current).map(groupOf),num=Array(81).fill(0),col=Array(10).fill(0);let sup=0;for(let i=0;i<draws.length-1;i++){const g=groupCounts(draws[i].balls).map(groupOf);let dist=0;for(let c=0;c<10;c++)dist+=Math.abs(g[c]-curG[c]);if(dist>6)continue;const w=1/(1+dist);sup+=w;const nxt=draws[i+1];for(const n of nxt.balls)num[n]+=w;if(nxt.column)col[nxt.column-1]+=2*w}
