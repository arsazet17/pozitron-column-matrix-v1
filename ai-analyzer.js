...
(() => {
  ...
  const ARCHIVE_KEY = 'pozitron_column_matrix_ai_archive_v1';
  const VERSION = 'AI-1.0';
  ...
  function normalizeDraw(d) {
    ...
    if (!Number.isFinite(draw) || !balls.every(n => n >= 1 && n <= 80)) return null;
    return {
      draw,
      date: String(d.date || ''),
      time: String(d.time || ''),
  ...
  function winner(draw) {
    ...
    for (let col = 1; col <= 10; col++) {
      ...
      let seen = 0;
      for (let i = 0; i < draw.balls.length; i++) {
        if (colOf(draw.balls[i]) === col) seen++;
        if (seen === max) { reached[col] = i; break; }
      }
    }
    let w = 1;
  ...
  function parityMode(wins) {
    ...
    if (last.length >= 7 && even >= Math.ceil(last.length * 0.7)) return { key:'EVEN', label:'ЧЁТ' };
    if (last.length >= 7 && (last.length - even) >= Math.ceil(last.length * 0.7)) return { key:'ODD', label:'НЕЧЁТ' };
  ...
  function train(draws) {
    ...
    const groupNext = Array.from({length:11}, () => ({'0':[1,1], '1':[1,1], '2':[1,1], '3':[1,1], '4+':[1,1]}));
    ...
    for (let i = 0; i < draws.length - 1; i++) {
      ...
      for (let col = 1; col <= 10; col++) {
        ...
        groupNext[col][g][1]++; // exposure
        if (b === col) groupNext[col][g][0]++; // hit
      }
  ...
  function addSignal(state, col, points, reason) {
    ...
    if (reason && !state[col].reasons.includes(reason)) state[col].reasons.push(reason);
  }

  function predict(draws) {
    const wins = draws.map(winner);
    ...
    // 1) Переход от последнего столба, обученный на всей истории.
    if (last) {
      const row = model.trans[last];
      const base = row.slice(1).reduce((a,b)=>a+b,0) / 10;
      for (let col=1; col<=10; col++) {
        const rel = row[col] / Math.max(base, 1);
        addSignal(state, col, Math.max(-5, Math.min(8, (rel - 1) * 22)), 'история переходов');
      }
    }
    ...
    // 3) Малое и большое зеркало последнего выхода.
    if (last && SMALL_MIRROR[last]) addSignal(state, SMALL_MIRROR[last], 9, 'малое зеркало');
    if (last && BIG_MIRROR[last]) addSignal(state, BIG_MIRROR[last], 7, 'большое зеркало');

    // 4) Продолжение шага и отскок.
    if (last && p1) {
      const step = last - p1;
      const cont = last + step;
      const bounce = last - step;
      if (cont >= 1 && cont <= 10 && step !== 0) addSignal(state, cont, 9, 'продолжение шага');
      if (bounce >= 1 && bounce <= 10 && step !== 0) addSignal(state, bounce, 7, 'обратный шаг');
      if (Math.abs(step) === 1) {
        const neighbor = last + (step > 0 ? 1 : -1);
        if (neighbor >= 1 && neighbor <= 10) addSignal(state, neighbor, 3, 'движение по соседям');
      }
    }
    ...
    // 6) Текущая группа 0/1/2/3/4+ и то, как из неё столб выходил исторически.
    for (let col=1; col<=10; col++) {
      const g = groupLabel(latestCounts[col] || 0);
      const [hit, exposure] = model.groupNext[col][g];
      const rate = hit / Math.max(exposure,1);
      addSignal(state, col, Math.max(-4, Math.min(7, (rate - 0.1) * 55)), `группа ${g}`);
    }

    // 7) Интервал с последнего выхода относительно собственной истории.
    for (let col=1; col<=10; col++) {
      let gap = wins.length;
      for (let i=wins.length-1; i>=0; i--) {
        if (wins[i] === col) { gap = wins.length - 1 - i; break; }
      }
  ...
  function ensureCurrentForecast(draws, archive) {
    ...
    const chain = draws.slice(-5).map(d => ({draw:d.draw, winner:winner(d)}));
    const rec = {
  ...
  function injectUi() {
    ...
    style.textContent = `
      ...
      .ai-good{color:#6ee7a0;font-weight:950}.ai-bad{color:#ff9b9b;font-weight:950}.ai-wait{color:#ffd34f;font-weight:950}
      @media(max-width:520px){.ai-picks{grid-template-columns:1fr}.viewtabs{grid-template-columns:1fr 1fr 1fr!important}.viewtab{font-size:12px;padding:8px 4px}}
    `;
    ...
    if (host) {
      ...
      section.innerHTML = `
        <div class="card ai-card">
          <div class="ai-head">
            <div><b>🧠 ИИ Анализ</b><div class="ai-sub">логический прогноз по текущему состоянию</div></div>
            <div id="aiMode" class="ai-mode">—</div>
          </div>
          <div id="aiPicks" class="ai-picks"></div>
          <div class="groupnote">Проценты не показываются. ИИ учитывает повторы, зеркала, шаги, группы и Ч/Н/МИКС.</div>
  ...
  function resultLabel(rec) {
    if (!rec.settled) return '<span class="ai-wait">⏳ ждём результат</span>';
    if (rec.result === 'TOP1') return '<span class="ai-good">✅ ТОП-1</span>';
    if (rec.result === 'TOP3') return '<span class="ai-good">✅ ТОП-3</span>';
  ...
  function render() {
    ...
    if (draws.length < 6) {
      if ($('aiPicks')) $('aiPicks').innerHTML = '<div class="row">Недостаточно тиражей для ИИ.</div>';
      return;
    }
    ...
    $('aiPicks').innerHTML = current.picks.map((col, i) => {
      const reasons = (current.reasons?.[col] || []).slice(0,3).join(' · ') || 'совокупный сигнал';
      return `<div class="ai-pick"><span>ТОП-${i+1}</span><strong>ст${col}</strong><small>${reasons}</small></div>`;
    }).join('');

    const recent = loadArchive().slice(-20).reverse();
    $('aiArchive').innerHTML = recent.map(rec => {
      const chain = (rec.chain || []).map(x => `№${x.draw}:ст${x.winner}`).join(' → ');
      ...
      return `<div class="ai-rec">
        ...
        <div>Прогноз: <b>${rec.picks.map(x=>'ст'+x).join(' · ')}</b>${fact}</div>
  ...
  // После автообновления matrix.js локальная база меняется без события.
  // Поэтому ИИ тихо проверяет её раз в 15 секунд.
